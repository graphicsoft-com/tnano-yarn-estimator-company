...
let Materials = [];
Materials[0] = new Material("Silicon Wafers", (waferSize === 3 ? 50 : 120));
Materials[1] = new Material("PCB Plates", 50 / speakersPerWafer); // PCB substrate material
Materials[2] = new Material("CNT Sheets", 100 / speakersPerWafer); // Carbon nanotube sheets
Materials[3] = new Material("Copper for Electrodes", 10 / speakersPerWafer); // Terminal electrode material
Materials[4] = new Material("Etching Solution", 15 / speakersPerWafer); // For PCB processing
Materials[5] = new Material("Conductive Adhesive", 20 / speakersPerWafer); // For attaching CNT sheets
Materials[6] = new Material("Frame Components", 30); // Housing and support structure
Materials[7] = new Material("Connection Components", 25); // Wiring and connectors
Materials[8] = new Material("Packaging Materials", 10); // Final product packaging

const workerTasks = [];
workerTasks[0] = new WorkerTask("CNT Sheet Preparation", 1 / speakersPerWafer);
workerTasks[1] = new WorkerTask("Electrode Deposition",1 / 8 / speakersPerWafer);
workerTasks[2] = new WorkerTask("CNT Sheet Mounting", 20 / 60);
workerTasks[3] = new WorkerTask("Terminal Connection", 15 / 60);
workerTasks[4] = new WorkerTask("Housing Assembly", 1);
workerTasks[5] = new WorkerTask("Final Assembly", 1);
workerTasks[6] = new WorkerTask("Acoustic Testing", 30 / 60);
workerTasks[7] = new WorkerTask("Quality Control", 15 / 60);
...
function updateWaferParameters() {
  ...
  const scalingFactor = oldSpeakersPerWafer / speakersPerWafer;
  for (let i = 1; i <= 5; i++) {
    Materials[i].costPerSpeaker *= scalingFactor;
  }
...
function CompareEquipment(e1, e2) {
  if (e1.maxThroughput < e2.maxThroughput) return 1;
  else return -1;
}
...
function RunCalculator() {
  ...
  if (document.getElementById("productionMode").value == 0) {
    ...
    types.forEach((type) => {
      ...
      let indexOfFinal = equipment.indexOf(
        GetEquipmentOfType(type)[GetEquipmentOfType(type).length - 1]
      );
    ...
    for (let i = 0; i < equipment.length; i++) {
      totalEquipmentCost +=
        equipment[i].cost *
        Math.pow(countOfEquipment[i], equipment[i].costExponent);
    }

    for (let i = 0; i < Materials.length - 1; i++) {
      totalMaterialCosts +=
        Materials[i].costPerSpeaker * speakersPerMonth * months;
    }

    for (let i = 0; i < workerTasks.length; i++) {
      totalLaborCosts +=
        workerTasks[i].taskTime *
        speakersPerMonth *
        months *
        hourlySalaryEngineer;
    }
  } else {
  ...
  DrawResultsBarChart(
    Math.round(totalEquipmentCost),
    Math.round(totalLaborCosts / months),
    Math.round(totalMaterialCosts / months),
    Math.round(totalIndirectCosts / months),
  ...
  if (indirectDetailHolder.hasChildNodes()) {
    ...
    for (let i = 0; i < indirectCosts.length; i++) {
      ...
      paragraphB.textContent = new Intl.NumberFormat("en-US", {
        ...
        Math.round(
          indirectCosts[i].monthlyCost *
            months *
            PerUnitFactor("totalCostUnit", wafersPerMonth, months)
        )
      );
...
function DrawText(
  ...
  UpdateTextContent(
    ...
    }).format(Math.round(pricePerSpeaker))
  );
  ...
  UpdateTextContent(
    ...
    new Intl.NumberFormat("en-US", {
      ...
      Math.round(
        totalLaborCosts * PerUnitFactor("totalCostUnit", wafersPerMonth, months)
      )
    )
  );
  ...
  UpdateTextContent(
    ...
    new Intl.NumberFormat("en-US", {
      ...
      Math.round(
        ...
        (1 + markup / 100) *
        PerUnitFactor("revenueUnit", wafersPerMonth, months)
    )
  );
  ...
  UpdateTextContent(
    ...
    new Intl.NumberFormat("en-US", {
      ...
      Math.round(
        totalProfits * PerUnitFactor("profitUnit", wafersPerMonth, months)
      )
    )
  );
}
...
function DrawResultsBarChart(
  ...
  for (let i = 0; i < months; i++) {
    ...
    if (i > 0) {
      profit[i] =
        profit[i - 1] + labor[i] + material[i] + indirect[i] + revenue[i];
    } else {
      profit[i] =
        labor[i] + material[i] + indirect[i] + revenue[i] - equipmentCost;
    }
  }
...
function DrawCostPieChart(
  ...
  let values = [
    ...
    Math.round(indirectCost),
  ];

  const barColors = ["#FF7719", "#73937E", "#585563", "#0AD3FF"];
...
function PricePerSpeaker(number) {
  ...
  if (document.getElementById("productionMode").value == 0) {
    ...
    for (let i = 0; i < equipment.length; i++) {
      totalEquipmentCost +=
        equipment[i].cost * Math.pow(cOE[i], equipment[i].costExponent);
    }
    ...
    for (let i = 0; i < workerTasks.length; i++) {
      totalLaborCosts +=
        workerTasks[i].taskTime * number * months * hourlySalaryEngineer;
    }
  } else {
    for (let i = 5; i < Materials.length; i++) {
      totalMaterialCosts += Materials[i].costPerSpeaker * number * months;
